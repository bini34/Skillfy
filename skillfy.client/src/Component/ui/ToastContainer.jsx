...
export default function ToastContainer() {
  ...
  return (
    <div
      aria-live="polite"
      className="fixed bottom-4 right-4 z-80 flex flex-col gap-2 max-w-sm w-full"
    >
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className="flex items-start gap-3 rounded-lg bg-white border border-gray-200 shadow-modal p-4 animate-slide-up"
          role="alert"
        >
          <span className="flex-shrink-0">{iconByType[toast.type] || iconByType.info}</span>
